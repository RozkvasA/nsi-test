...
export function ParameterContent({
  ...
  if (selectedRef.kind === 'object') {
    ...
    const linkedEquipment = equipment.filter((item) => item.placementObjectId === object.id || linkedSystemIds.has(item.systemId));
    ...
    if (activeGroupId === 'relations') {
      return (
        <div className="parameter-section">
          ...
          <RelationBlock title="Связанные системы" description="Для помещения меняется linkedRoomIds, для другого узла дерева меняется scopeObjectIds. Глобальная система не дублируется ниже. Кнопка Открыть переводит в карточку системы." items={systems.map((system) => ({ id: system.id, label: system.name, checked: system.linkedRoomIds.includes(object.id) || system.scopeObjectIds.includes(object.id) }))} onToggle={(id) => onToggleObjectSystemLink(object.id, id)} onOpen={onSelectSystem} />
          <RelationBlock title="Связанное оборудование" description="Показывается оборудование, размещенное на объекте, и оборудование систем, связанных с объектом. Кнопка Открыть переводит в карточку оборудования." items={equipment.map((item) => ({ id: item.id, label: item.name, checked: linkedEquipment.some((linkedItem) => linkedItem.id === item.id) }))} onToggle={(id) => onToggleEquipmentPlacement(object.id, id)} onOpen={onSelectEquipment} />
          ...
          <div className="bulk-panel"><b>Массовая привязка</b><p>Для выбранного объекта можно привязать систему ко всем помещениям ветки. Действие сохраняет систему отдельной сущностью.</p><button type="button" onClick={() => systems[0] && onBulkLinkRoomsToSystem(systems[0].id, roomsInBranch.map((room) => room.id))}>Привязать первую систему ко всем помещениям</button></div>
        </div>
      );
    }

    if (activeGroupId === 'additional') {
      return <div className="parameter-section"><SectionTitle title="Прочие параметры и служебные поля" description="Здесь показаны параметры вида вне основной группы и технические идентификаторы объекта." /><InfoGrid items={[{ label: 'id', value: object.id }, { label: 'typeId', value: object.typeId }, { label: 'parentId', value: object.parentId ?? 'Нет' }, { label: 'status', value: object.status }, { label: 'rootObjectId', value: detailInfo.rootObjectId }]} />{additionalTypeParameters.length > 0 ? <div className="dynamic-parameter-list">{additionalTypeParameters.map((parameter) => <ObjectParameterInput key={parameter.id} parameter={parameter} value={getObjectParameterValue(object, parameter)} onChange={(value) => updateObjectParameterValue(parameter, value)} />)}</div> : <EmptyState title="Прочих параметров вида нет" description="Добавьте их в разделе Дерево видов объектов, вкладка Прочие." />}{orphanParameters.length > 0 ? <KeyValueList showEmpty={showEmpty} rows={orphanParameters.map(([label, value]) => ({ label, value }))} /> : null}</div>;
    }

    return <div className="parameter-section"><SectionTitle title="Основные параметры объекта" description="Редактирование выполняется в правой карточке. Вид объекта берется из актуального Дерева видов объектов." />{requiredWarnings.length > 0 ? <div className="inline-warning"><b>Есть обязательные незаполненные параметры</b><p>{requiredWarnings.join('; ')}</p></div> : null}<EditableField label="Идентификатор" value={object.id} readOnly /><EditableField label="Наименование" value={object.name} onChange={(value) => onUpdateObject(object.id, { name: value })} /><EditableField label="Сокращение" value={object.shortName} onChange={(value) => onUpdateObject(object.id, { shortName: value })} /><SelectField label="Вид объекта" value={object.typeId} options={objectTypes.map((type) => ({ value: type.id, label: `${type.icon} ${type.name}` }))} onChange={(value) => onUpdateObject(object.id, { typeId: value })} /><SelectField label="Родительский объект" value={object.parentId ?? ''} options={parentOptions} onChange={(value) => onUpdateObject(object.id, { parentId: value || null })} /><EditableField label="Площадь" value={object.area ?? ''} type="number" empty={object.area === null} showEmpty={showEmpty} onChange={(value) => onUpdateObject(object.id, { area: value === '' ? null : Number(value) })} /><EditableField label="Количество" value={object.quantity} type="number" onChange={(value) => onUpdateObject(object.id, { quantity: Number(value) })} /><EditableField label="Единица измерения" value={object.unit} onChange={(value) => onUpdateObject(object.id, { unit: value })} /><SelectField label="Статус" value={object.status} options={[{ value: 'active', label: 'На учете' }, { value: 'retired', label: 'Снят с учета' }]} onChange={(value) => onUpdateObject(object.id, { status: value as ObjectStatus })} />{isRootObject ? <EditableField label="Уровень детализации" value={detailInfo.detailLevel} type="number" onChange={(value) => onUpdateObject(object.id, { parameters: { ...object.parameters, detailLevel: Math.max(1, Number(value) || 1) } })} /> : null}<InfoGrid items={[{ label: 'Уровень объекта', value: detailInfo.objectLevel }, { label: 'Детализация начинается с уровня', value: detailInfo.detailLevel }, { label: 'Статус детализации', value: detailStatus }]} /><div className="dynamic-parameter-list"><div className="inline-title"><b>Параметры по виду объекта</b><small>{objectType?.name ?? 'Вид не найден'}</small></div>{mainTypeParameters.length > 0 ? mainTypeParameters.map((parameter) => <ObjectParameterInput key={parameter.id} parameter={parameter} value={getObjectParameterValue(object, parameter)} onChange={(value) => updateObjectParameterValue(parameter, value)} />) : <EmptyState title="Параметров в основной группе нет" description="Добавьте параметры в Дереве видов объектов или выберите другой вид." />}</div></div>;
  }
  ...
  if (activeGroupId === 'relations') return <div className="parameter-section"><SectionTitle title="Связи справочника" description="Справочники отображаются тем же паттерном: дерево слева, карточка справа." /><InfoGrid items={[{ label: 'Родитель', value: parent?.title ?? 'Нет' }, { label: 'Дочерних записей', value: children.length }]} /></div>;
