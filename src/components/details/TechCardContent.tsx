...
export function TechCardContent({ card, objectTypes, dictionaries, activeTab, onSetActiveTab, onUpdateTechCard }: TechCardContentProps) {
  ...
  return (
    <div className="tech-card-editor">
      ...
      {warnings.length > 0 ? <div className="inline-warning tech-card-warning"><b>Предупреждения по техкарте</b><p>{warnings.join('; ')}</p></div> : null}

      {safeActiveTab === 'Параметры' ? <TechCardParameters card={card} objectTypes={objectTypes} dictionaries={dictionaries} patchCard={patchCard} /> : null}
...
function TechCardParameters({ card, objectTypes, dictionaries, patchCard }: { card: TechCard; objectTypes: ObjectType[]; dictionaries: DictionaryItem[]; patchCard: (patch: Partial<TechCard>) => void }) {
...
function TechCardOperations({ card, patchCard }: { card: TechCard; patchCard: (patch: Partial<TechCard>) => void }) {
  const updateOperation = (operationId: string, patch: Partial<TechCardOperation>) => patchCard({ operations: card.operations.map((operation) => (operation.id === operationId ? { ...operation, ...patch } : operation)) });
  const deleteOperation = (operationId: string) => patchCard({ operations: card.operations.filter((operation) => operation.id !== operationId).map((operation, index) => ({ ...operation, order: index + 1 })) });
  const totalLabor = card.personnel.reduce((sum, row) => sum + row.count * (row.minDurationManHours ?? 0), 0);
  const duration = card.minDurationManHours ?? totalLabor;

  return (
    <div className="parameter-section reference-table-section">
      <SectionTitle title="Операции" description="Состав работ редактируется прямо в карточке техкарты." />
      <button type="button" className="secondary-action" onClick={() => patchCard({ operations: [...card.operations, createEmptyOperation(card.operations.length + 1)] })}>Добавить операцию</button>
      ...
      {card.operations.slice().sort((a, b) => a.order - b.order).map((operation, index) => (
        <div className="tech-card-row operation-row" key={operation.id}>
          <div className="inline-title"><b>Этап {operation.order}</b><div className="row-actions-inline"><button type="button" onClick={() => patchCard({ operations: moveOperation(card.operations, operation.id, 'up') })} disabled={index === 0}>Выше</button><button type="button" onClick={() => patchCard({ operations: moveOperation(card.operations, operation.id, 'down') })} disabled={index === card.operations.length - 1}>Ниже</button><button type="button" onClick={() => deleteOperation(operation.id)}>Удалить</button></div></div>
          <div className="reference-fields-grid">
      ...
      <div className="summary-strip"><span>Операций: {card.operations.length}</span><span>Общая длительность: {duration || 'не задана'} чел-ч</span><span>Трудозатраты персонала: {totalLabor || 'не заданы'} чел-ч</span></div>
    </div>
  );
}
...
function TechCardPersonnelTab({ card, dictionaries, patchCard }: { card: TechCard; dictionaries: DictionaryItem[]; patchCard: (patch: Partial<TechCard>) => void }) {
  ...
  const updatePersonnel = (rowId: string, patch: Partial<TechCardPersonnel>) => patchCard({ personnel: card.personnel.map((row) => (row.id === rowId ? { ...row, ...patch } : row)) });
  return (
    <div className="parameter-section reference-table-section">
...
function TechCardMaterials({ card, dictionaries, patchCard }: { card: TechCard; dictionaries: DictionaryItem[]; patchCard: (patch: Partial<TechCard>) => void }) {
  ...
  return (
    <div className="parameter-section reference-table-section">
      <SectionTitle title="Материалы" description="Материалы и расходники. Список может быть осознанно пустым, если материалы не требуются. Цены в текущих данных не заведены." />
...
function TechCardPpeTab({ card, dictionaries, patchCard }: { card: TechCard; dictionaries: DictionaryItem[]; patchCard: (patch: Partial<TechCard>) => void }) {
  ...
  return (
    <div className="parameter-section reference-table-section">
      <SectionTitle title="СИЗ" description="Средства индивидуальной защиты. Если СИЗ не требуются, список может быть осознанно пустым." />
