...
export function EquipmentContent({ equipmentItem, equipment, systems, objects, objectTypes, techCards, activeTab, onSetActiveTab, onUpdateEquipment, onCreateEquipmentType, onAddChildEquipment, onCreateMissingChildUnits, onCreateChildUnitsFromRows, onRemoveChildUnits, onSelectEquipment, onSelectSystem, onSelectTechCard, onCreateTechCardForEquipment }: EquipmentContentProps) {
  ...
  return (
    <div className="tech-card-editor equipment-card-editor">
      ...
      {safeActiveTab === 'Вложенность' ? <EquipmentNesting equipmentItem={equipmentItem} equipment={equipment} systems={systems} objects={objects} objectTypes={objectTypes} patchEquipment={patchEquipment} onUpdateEquipment={onUpdateEquipment} onAddChildEquipment={onAddChildEquipment} onCreateMissingChildUnits={onCreateMissingChildUnits} onCreateChildUnitsFromRows={onCreateChildUnitsFromRows} onRemoveChildUnits={onRemoveChildUnits} onSelectEquipment={onSelectEquipment} /> : null}
...
function EquipmentNesting({ equipmentItem, equipment, systems, objects, objectTypes, patchEquipment, onUpdateEquipment, onAddChildEquipment, onCreateMissingChildUnits, onCreateChildUnitsFromRows, onRemoveChildUnits, onSelectEquipment }: { equipmentItem: EquipmentEntity; equipment: EquipmentEntity[]; systems: SystemEntity[]; objects: InfrastructureObject[]; objectTypes: ObjectType[]; patchEquipment: (patch: Partial<EquipmentEntity>) => void; onUpdateEquipment: (id: string, patch: Partial<EquipmentEntity>) => void; onAddChildEquipment: (parentEquipmentId: string) => void; onCreateMissingChildUnits: (parentEquipmentId: string) => void; onCreateChildUnitsFromRows: (parentEquipmentId: string, rows: Array<{ name: string; inventoryNumber?: string }>) => void; onRemoveChildUnits: (unitIds: string[]) => number; onSelectEquipment: (equipmentId: string) => void }) {
