...
function EquipmentNesting({ equipmentItem, equipment, systems, objects, objectTypes, patchEquipment, onUpdateEquipment, onAddChildEquipment, onCreateMissingChildUnits, onSelectEquipment }: { equipmentItem: EquipmentEntity; equipment: EquipmentEntity[]; systems: SystemEntity[]; objects: InfrastructureObject[]; objectTypes: ObjectType[]; patchEquipment: (patch: Partial<EquipmentEntity>) => void; onUpdateEquipment: (id: string, patch: Partial<EquipmentEntity>) => void; onAddChildEquipment: (parentEquipmentId: string) => void; onCreateMissingChildUnits: (parentEquipmentId: string) => void; onSelectEquipment: (equipmentId: string) => void }) {
  ...
  const parsePasteRows = () => pasteFillText.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => { const parts = line.includes('\t') ? line.split('\t') : line.split(';'); return { name: (parts[0] ?? '').trim(), inventoryNumber: parts.length > 1 ? (parts[1] ?? '').trim() : undefined }; }).filter((row) => row.name.length > 0);
  const applyPasteFill = () => { const rows = parsePasteRows(); const appliedRows = rows.slice(0, unitChildren.length); appliedRows.forEach((row, index) => { const child = unitChildren[index]; const patch: Partial<EquipmentEntity> = { name: row.name }; if (row.inventoryNumber !== undefined) patch.parameters = { ...child.parameters, inventoryNumber: row.inventoryNumber }; onUpdateEquipment(child.id, patch); }); setPasteFillResult(`Применено строк: ${appliedRows.length} из ${rows.length}.`); setPasteFillText(''); setIsPasteFillOpen(false); resetUnitSelection(); };
