...
function EquipmentNesting({ equipmentItem, equipment, systems, objects, objectTypes, patchEquipment, onUpdateEquipment, onReorderChildUnit, onAddChildEquipment, onCreateMissingChildUnits, onCreateChildUnitsFromRows, onRemoveChildUnits, onSelectEquipment }: { equipmentItem: EquipmentEntity; equipment: EquipmentEntity[]; systems: SystemEntity[]; objects: InfrastructureObject[]; objectTypes: ObjectType[]; patchEquipment: (patch: Partial<EquipmentEntity>) => void; onUpdateEquipment: (id: string, patch: Partial<EquipmentEntity>) => void; onReorderChildUnit: (parentEquipmentId: string, unitId: string, direction: 'up' | 'down') => void; onAddChildEquipment: (parentEquipmentId: string) => void; onCreateMissingChildUnits: (parentEquipmentId: string) => void; onCreateChildUnitsFromRows: (parentEquipmentId: string, rows: Array<{ name: string; inventoryNumber?: string }>) => void; onRemoveChildUnits: (unitIds: string[]) => number; onSelectEquipment: (equipmentId: string) => void }) {
  ...
  const openBulkAction = (action: SelectedBulkAction) => { setBulkAction(action); setChildUnitActionResult(''); if (action !== 'placement') setSelectedPlacementObjectId(''); if (action !== 'system') setSelectedSystemId(null); if (action !== 'type') setSelectedTypeId(''); };
  ...
  const syncSelectedUnitsWithParent = () => { selectedUnitChildren.forEach((child) => onUpdateEquipment(child.id, { systemId: equipmentItem.systemId, placementObjectId: equipmentItem.placementObjectId, typeId: equipmentItem.typeId, quantity: 1, unit: 'шт.' })); closeBulkAction(); resetUnitSelection(); };
  const renameSelectedUnitsByTemplate = () => { selectedUnitChildren.forEach((child, index) => onUpdateEquipment(child.id, { name: `${baseName} ${String(index + 1).padStart(3, '0')}` })); closeBulkAction(); resetUnitSelection(); };
  const clearSelectedInventoryNumbers = () => { selectedUnitChildren.forEach((child) => onUpdateEquipment(child.id, { parameters: { ...child.parameters, inventoryNumber: '' } })); closeBulkAction(); resetUnitSelection(); };
  const applySelectedPlacement = () => { if (!selectedPlacementObjectId) return; selectedUnitChildren.forEach((child) => onUpdateEquipment(child.id, { placementObjectId: selectedPlacementObjectId })); closeBulkAction(); setChildUnitActionResult(`Размещение изменено у выбранных unit: ${selectedCount}.`); resetUnitSelection(); };
  const applySelectedSystem = () => { if (selectedSystemId === null) return; selectedUnitChildren.forEach((child) => onUpdateEquipment(child.id, { systemId: selectedSystemId })); closeBulkAction(); setChildUnitActionResult(`Система изменена у выбранных unit: ${selectedCount}.`); resetUnitSelection(); };
  const applySelectedType = () => { if (!selectedTypeId) return; selectedUnitChildren.forEach((child) => onUpdateEquipment(child.id, { typeId: selectedTypeId })); closeBulkAction(); setChildUnitActionResult(`Вид изменен у выбранных unit: ${selectedCount}.`); resetUnitSelection(); };
  const reconcileParentQuantityToFact = () => patchEquipment({ quantity: unitChildren.length });
  const confirmRemoveExtraUnits = () => { const removedCount = onRemoveChildUnits(extraUnitChildren.map((child) => child.id)); setIsExtraUnitDeleteOpen(false); setChildUnitActionResult(`Удалено лишних unit: ${removedCount}.`); resetUnitSelection(); };
  const confirmRemoveSelectedUnits = () => { const removedCount = onRemoveChildUnits(selectedUnitRows.map(({ child }) => child.id)); closeBulkAction(); setChildUnitActionResult(`Удалено выбранных unit: ${removedCount}.`); resetUnitSelection(); };
  ...
  const applyPasteFill = () => { const rowsToApply = applicablePasteRows; rowsToApply.forEach((row) => { if (!row.unit) return; const patch: Partial<EquipmentEntity> = { name: row.name }; if (row.hasInventoryColumn) patch.parameters = { ...row.unit.parameters, inventoryNumber: row.inventoryNumber ?? '' }; onUpdateEquipment(row.unit.id, patch); }); setPasteFillResult(`Применено строк: ${rowsToApply.length} из ${pastePreviewRows.length}.`); setPasteFillText(''); setIsPasteFillOpen(false); closeBulkAction(); resetUnitSelection(); };
  const createUnitsFromExtraRows = () => { const rowsToCreate = extraRowsForCreate.map((row) => ({ name: row.name, inventoryNumber: row.hasInventoryColumn ? row.inventoryNumber ?? '' : undefined })); onCreateChildUnitsFromRows(equipmentItem.id, rowsToCreate); setPasteFillResult(`Создано unit из лишних строк: ${rowsToCreate.length}.`); setPasteFillText(''); setIsPasteFillOpen(false); closeBulkAction(); resetUnitSelection(); };
