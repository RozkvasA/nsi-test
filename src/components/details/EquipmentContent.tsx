    ...
    <div className="parameter-section reference-table-section">
      ...
      {isQuantityParentLevel ? (
        <div className="tech-card-row">
          <div className="inline-title"><b>Контроль количества</b>{canReconcileParentQuantity ? <button type="button" onClick={reconcileParentQuantityToFact}>Привести количество родителя к факту</button> : null}</div>
          <div className="reference-fields-grid">
      ...
      {canManageUnitList ? (
        <div className="tech-card-row">
          <div className="inline-title"><b>Дочерние единицы</b><span>Выбрано: {selectedCount} из {unitChildren.length}</span>{unitChildren.length > 0 ? <button type="button" onClick={renameUnitsByTemplate}>Переименовать по шаблону</button> : null}<button type="button" onClick={() => { setIsPasteFillOpen(true); setPasteFillResult(''); }}>Заполнить из списка</button></div>
          {selectedCount > 0 ? <div className="relation-actions system-fast-actions"><button type="button" className="secondary-action" onClick={syncSelectedUnitsWithParent}>Синхронизировать выбранные с родителем</button><button type="button" className="secondary-action" onClick={renameSelectedUnitsByTemplate}>Переименовать выбранные по шаблону</button><button type="button" className="secondary-action" onClick={clearSelectedInventoryNumbers}>Очистить инвентарные номера выбранных</button></div> : null}
