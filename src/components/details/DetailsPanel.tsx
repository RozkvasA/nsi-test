  ...
  return (
    <section className="details-panel">
      <header className="details-header">
        ...
        <span>{pendingObjectDraft ? 'Заполните основные данные. Дальше можно добавить системы, помещения или оборудование.' : selectedEntity?.subtitle ?? 'Выберите строку в дереве'}</span>
      </header>
      ...
      {pendingObjectDraft ? (
        <div className="create-object-panel calm-create-panel">
          <div className="section-title">
            <h3>{isRootDraft ? 'Новый корневой объект' : pendingObjectDraft.kind === 'room' ? 'Новое помещение' : 'Новый элемент дерева'}</h3>
            <p>Минимальный набор полей. Порядок заполнения не навязывается.</p>
          </div>
          {isRootDraft ? (
            <div className="creation-mode-card">
              <span>Способ создания</span>
              <div className="mode-buttons">
                <button type="button" className={pendingObjectDraft.creationMode === 'empty' ? 'mode-button active' : 'mode-button'} onClick={() => onUpdatePendingObjectDraft({ creationMode: 'empty' })}>Пустой объект</button>
                <button type="button" className={pendingObjectDraft.creationMode === 'template' ? 'mode-button active' : 'mode-button'} onClick={() => onUpdatePendingObjectDraft({ creationMode: 'template' })}>Из шаблона</button>
              </div>
            </div>
          ) : null}
          {isRootDraft && pendingObjectDraft.creationMode === 'template' ? (
            <>
              ...
              <div className="template-description"><b>{selectedTemplate?.name ?? 'Шаблон не выбран'}</b><p>{selectedTemplate?.description ?? 'Выберите шаблон структуры объекта.'}</p></div>
            </>
          ) : null}
          <label className="field-row"><span>Родительский объект</span><input value={parentObject?.name ?? 'Корневой уровень'} readOnly /></label>
          <label className="field-row"><span>Вид элемента</span><select value={pendingObjectDraft.typeId} onChange={(event) => onUpdatePendingObjectDraft({ typeId: event.target.value })}>{objectTypes.map((type) => <option key={type.id} value={type.id}>{type.icon} {type.name}</option>)}</select></label>
          ...
          <div className="create-actions"><button type="button" onClick={onConfirmCreateObject}>Создать</button><button type="button" onClick={onCreateObjectTypeForDraft}>Создать новый вид</button><button type="button" onClick={onCancelPendingObjectDraft}>Отмена</button></div>
        </div>
      ) : selectedSystem ? (
