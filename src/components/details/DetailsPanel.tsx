...
export function DetailsPanel(props: DetailsPanelProps) {
  ...
  return (
    <section className="details-panel">
      <header className="details-header"><p className="eyebrow">Карточка элемента</p><h2>{pendingObjectDraft ? 'Создание объекта учета' : selectedEntity?.title ?? 'Элемент не выбран'}</h2><span>{pendingObjectDraft ? 'Выберите способ создания и вид объекта' : selectedEntity?.subtitle ?? 'Выберите строку в дереве'}</span></header>
      ...
      {pendingObjectDraft ? (
        ...
        <><div className="tabs">{tabs.map((tab) => <button key={tab} type="button" className={tab === activeTab ? 'tab active' : 'tab'} onClick={() => onSetActiveTab(tab)}>{tab}</button>)}</div>{activeTab === 'Параметры' ? <div className="parameter-card parameter-card-wide"><ParameterContent selectedRef={selectedRef} selectedEntity={selectedEntity} activeGroupId={activeGroupId} showEmpty={showEmpty} objects={objects} objectTypes={objectTypes} systems={systems} equipment={equipment} techCards={techCards} dictionaries={dictionaries} onCreate={onCreate} onUpdateObject={onUpdateObject} onUpdateObjectType={onUpdateObjectType} onToggleAllowedChildType={onToggleAllowedChildType} onAddParameterGroup={onAddParameterGroup} onRenameParameterGroup={onRenameParameterGroup} onAddParameterToGroup={onAddParameterToGroup} onUpdateParameter={onUpdateParameter} onDeleteParameter={onDeleteParameter} onToggleObjectSystemLink={onToggleObjectSystemLink} onToggleEquipmentPlacement={onToggleEquipmentPlacement} onToggleSystemRoomLink={onToggleSystemRoomLink} onBulkLinkRoomsToSystem={onBulkLinkRoomsToSystem} onSelectSystem={(systemId) => onSelectSystem(systemId, selectedRef.kind === 'object' ? selectedRef.id : selectedContextObjectId)} onSelectEquipment={onSelectEquipment} onUpdateTechCard={onUpdateTechCard} /></div> : <div className="stub-tab"><h3>{activeTab}</h3><p>Раздел оставлен как заглушка этапа 1. Каркас вкладки есть, детальная логика будет подключаться следующими этапами.</p></div>}</>
      )}
    </section>
  );
}
