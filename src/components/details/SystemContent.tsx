...
export function SystemContent({ system, systems, objects, objectTypes, equipment, activeTab, contextObjectId, onSetActiveTab, onUpdateSystem, onCreateSystemType, onAddEquipmentToSystem, onDetachEquipmentFromSystem, onSelectEquipment, onSelectSystem, onLinkSystemToContextObject, onLinkSystemToRoomsInContext }: SystemContentProps) {
  ...
  return <div className="tech-card-editor system-card-editor"><div className="tabs system-tabs">{systemTabs.map((tab) => <button key={tab} type="button" className={tab === safeActiveTab ? 'tab active' : 'tab'} onClick={() => onSetActiveTab(tab)}>{tab}</button>)}</div>{warnings.length > 0 ? <div className="inline-warning system-warning"><b>Предупреждения по системе</b><p>{warnings.join('; ')}</p></div> : null}{safeActiveTab === 'Параметры' ? <SystemParameters system={system} systems={systems} objectTypes={objectTypes} patchSystem={patchSystem} onCreateSystemType={onCreateSystemType} /> : null}{safeActiveTab === 'Связи' ? <SystemRelations system={system} objects={objects} objectTypes={objectTypes} contextObjectId={contextObjectId} patchSystem={patchSystem} onLinkSystemToContextObject={onLinkSystemToContextObject} onLinkSystemToRoomsInContext={onLinkSystemToRoomsInContext} /> : null}{safeActiveTab === 'Оборудование' ? <SystemEquipment system={system} objects={objects} objectTypes={objectTypes} equipment={equipment} onAddEquipmentToSystem={onAddEquipmentToSystem} onDetachEquipmentFromSystem={onDetachEquipmentFromSystem} onSelectEquipment={onSelectEquipment} /> : null}{safeActiveTab === 'Документы' || safeActiveTab === 'Заметки' ? <div className="stub-tab"><h3>{safeActiveTab}</h3><p>Заглушка карточки системы. Детальная логика будет подключаться отдельным этапом.</p></div> : null}{systems.filter((item) => item.parentSystemId === system.id).length > 0 ? <div className="parameter-section system-children-block"><SectionTitle title="Дочерние системы" description="Вложенность систем хранится через родительскую систему, а не через место размещения." /><div className="relation-selected-table">{systems.filter((item) => item.parentSystemId === system.id).map((child) => <div key={child.id} className="relation-selected-row"><span>{child.name}</span><button type="button" onClick={() => onSelectSystem(child.id)}>Открыть</button></div>)}</div></div> : null}</div>;
}
...
function SystemEquipment({ system, objects, objectTypes, equipment, onAddEquipmentToSystem, onDetachEquipmentFromSystem, onSelectEquipment }: { system: SystemEntity; objects: InfrastructureObject[]; objectTypes: ObjectType[]; equipment: EquipmentEntity[]; onAddEquipmentToSystem: (systemId: string) => void; onDetachEquipmentFromSystem: (systemId: string, equipmentId: string) => void; onSelectEquipment: (equipmentId: string) => void }) {
  ...
  return <div className="parameter-section reference-table-section"><SectionTitle title="Инженерная структура" description="Физическое размещение хранится у единиц оборудования, а система задает инженерный контур." /><button type="button" className="secondary-action" onClick={() => onAddEquipmentToSystem(system.id)}>Добавить единицу оборудования</button>{systemEquipment.length === 0 ? <EmptyState title="Пустая система" description="Система создана как черновик. Добавьте оборудование, чтобы инженерный контур был заполнен." /> : null}<div className="engineering-tree">{roots.map((item) => <EquipmentTreeNode key={item.id} item={item} allEquipment={systemEquipment} objects={objects} objectTypes={objectTypes} level={0} onDetachEquipmentFromSystem={(equipmentId) => onDetachEquipmentFromSystem(system.id, equipmentId)} onSelectEquipment={onSelectEquipment} />)}</div></div>;
}
...
function EquipmentTreeNode({ item, allEquipment, objects, objectTypes, level, onDetachEquipmentFromSystem, onSelectEquipment }: { item: EquipmentEntity; allEquipment: EquipmentEntity[]; objects: InfrastructureObject[]; objectTypes: ObjectType[]; level: number; onDetachEquipmentFromSystem: (equipmentId: string) => void; onSelectEquipment: (equipmentId: string) => void }) {
  ...
  return <div className="engineering-tree-node" style={{ marginLeft: level * 18 }}><div className="engineering-tree-row"><div><b>{item.name}</b><small>{typeLabel(objectTypes, item.typeId)} · место: {objectLabel(objects, item.placementObjectId)} · {item.quantity} {item.unit}</small></div><div className="row-actions-inline"><button type="button" onClick={() => onSelectEquipment(item.id)}>Открыть</button><button type="button" onClick={() => onDetachEquipmentFromSystem(item.id)}>Сделать самостоятельным</button></div></div>{children.map((child) => <EquipmentTreeNode key={child.id} item={child} allEquipment={allEquipment} objects={objects} objectTypes={objectTypes} level={level + 1} onDetachEquipmentFromSystem={onDetachEquipmentFromSystem} onSelectEquipment={onSelectEquipment} />)}</div>;
}
